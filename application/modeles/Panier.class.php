...
// <editor-fold defaultstate="collapsed" desc="région FONCTIONS GET">
    ...
    public static function getPrixByQteProduit($idProduit) {
        $prix = 0;
        if (array_key_exists($idProduit, self::getProduits()))
                $prix = (GestionBoutique::getProduitById($idProduit)->prix) * $_SESSION['produits'][$idProduit];
        return $prix;
    }
    
    public static function getPrixTotal() {
        $prixTotal = 0;
        foreach (self::getProduits() as $idProduit => $qte) {
            $prixTotal += self::getPrixByQteProduit($idProduit);
        }
        return $prixTotal;
    }
